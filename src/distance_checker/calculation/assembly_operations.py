...
class FindingBolt:
    ...
    def searching_assembly_parts(self, bolt_grade: str, bolt_diameter: int,
        ...
        searched_bolt = Bolt.objects.filter(diameter=bolt_diameter,
                                            standard__title__in=chosen_standard
                                            ).order_by("length")

        searched_nut = Nut.objects.filter(diameter=bolt_diameter,
                                          standard__title__in=chosen_standard
                                          ).first()

        searched_washer = Washer.objects.filter(diameter=bolt_diameter,
                                                standard__title__in=chosen_standard
                                                ).first()
        ...
        basic_clamping_length = 2 * thickness_washer + thickness_nut + \
                                2 * thickness_plate + thread_end
        ...
        if bolt_grade == "10_9":
            ...
            searched_bolt = searched_bolt.filter(
                length__gt=basic_clamping_length).first()
